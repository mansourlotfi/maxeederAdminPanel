...
import CommentsSearch from "./Search";

export default function AdminComments() {
  const { comments, isLoaded, status, metaData, messageParams } = useComments();
  ...
  const multipleItemsDeleteHandler = useCallback(() => {
    agent.Admin.CommentDeleteMultipleItems(checkedIds)
      .then(() => {
  ...
  const multipleItemsEditHandler = useCallback(() => {
    agent.Admin.CommentEditMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchCommentsAsync());
        toast.success("عملیات با موفقیت انجام شد");
      })
