...
export default function AdminSizes() {
  ...
  function handleDelete(id: number) {
    setLoading(true);
    setTarget(id);
    agent.Admin.deleteSize(id)
      .then(() => dispatch(removeSize(id)))
  ...
  const multipleItemsDeleteHandler = useCallback(() => {
    agent.Admin.SizesDeleteMultipleItems(checkedIds)
      .then(() => {
  ...
  const multipleItemsEditHandler = useCallback(() => {
    agent.Admin.SizesEditMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchSizesAsync());
        toast.success("عملیات با موفقیت انجام شد");
      })
              ...
              >
                ...
                <TableCell align="left">
                  <Box display="flex" alignItems="center">
                    <span>{D.sizeName}</span>
                  </Box>
                </TableCell>
                <TableCell align="left">
                  <Box display="flex" alignItems="center">
                    <span>{D.sizeNameEn}</span>
                  </Box>
                </TableCell>
                ...
                <TableCell align="left">
                  <Button
                    onClick={() => {
                      navigator.clipboard.writeText(D.sizeName);
