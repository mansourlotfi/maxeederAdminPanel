...
import { fetchUsagesAsync, removeUsage } from "./usageSlice";
...
export default function AdminUsagePage() {
  const { usages, isLoaded, status } = useUsages();
  ...
  function handleDelete(id: number) {
    setLoading(true);
    setTarget(id);
    agent.Admin.deleteUsage(id)
      .then(() => dispatch(removeUsage(id)))
  ...
  const multipleItemsDeleteHandler = useCallback(() => {
    agent.Admin.UsagesDeleteMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchUsagesAsync());
  ...
  const multipleItemsEditHandler = useCallback(() => {
    agent.Admin.UsagesEditMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchUsagesAsync());
        toast.success("عملیات با موفقیت انجام شد");
      })
