...
import QuickAccessMenuSearch from "./Search";

export default function AdminQuickAccessMenu() {
  const { quickAccessMenu, isLoaded, status, metaData, quickAccessMenuParams } =
    useQuickAccessMenu();
  ...
  const selectAllItem = (e: any) => {
    ...
    if (checked) {
      for (const item of quickAccessMenu) {
  ...
  const multipleItemsDeleteHandler = useCallback(() => {
    agent.Admin.quickAccessDeleteMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchQuickAccessMenuAsync());
  ...
  const multipleItemsEditHandler = useCallback(() => {
    agent.Admin.quickAccessEditMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchQuickAccessMenuAsync());
        toast.success("عملیات با موفقیت انجام شد");
      })
        ...
        <Grid container mt={5} justifyContent="space-between">
          ...
          <Grid item xs={1}>
            <FormControl fullWidth>
              ...
              <Select
                value={quickAccessMenuParams.pageSize}
                label="تعداد در صفحه"
                onChange={(e) =>
