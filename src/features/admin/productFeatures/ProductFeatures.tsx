...
export default function AdminProductFeatures() {
  const { productFeatures, isLoaded, status, metaData, productFeatureParams } =
    useProductFeatures();
  ...
  const multipleItemsDeleteHandler = useCallback(() => {
    agent.Admin.productFeaturesDeleteMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchProductFeaturesAsync());
  ...
  const multipleItemsEditHandler = useCallback(() => {
    agent.Admin.productFeaturesEditMultipleItems(checkedIds)
      .then(() => {
        dispatch(fetchProductFeaturesAsync());
        toast.success("عملیات با موفقیت انجام شد");
      })
              ...
              >
                <TableCell component="th" scope="row">
                  <Grid
                    ...
                    alignItems="center"
                  >
                    <Box display="inline-block">{PF.id}</Box>
        ...
        <Grid container mt={5} justifyContent="space-between">
          ...
          <Grid item xs={1}>
            <FormControl fullWidth>
              ...
              <Select
                value={productFeatureParams.pageSize}
                label="تعداد در صفحه"
                onChange={(e) =>
